...
//获取datepicker的配置
export function pickerOptions(options, moment) {
  if (!moment) {
    return {};
  }
  let lessThan = 0,
    moreThan = 0;
  if (options.limit == 'stepRange') {
    lessThan = options.stepRangeBefore || 0;
  }
  if (options.limit == 'stepRange') {
    moreThan = options.stepRangeAfter || 0;
  }
  ...
  return {
    date: {
      disabledDate(time) {
        ...
        start.setMinutes(59);
        start.setSeconds(59);
        if (options.limit === 'systemTimeBefore') {
          return time.getTime() > start;
        }
        if (options.limit === 'systemTimeAfter') {
          return time.getTime() < start;
        }
        if (options.limit === 'stepRange') {
          return (
            time.getTime() < now - lessThan || time.getTime() > now + moreThan
          );
        }
        return false;
      }
    ...
    time: {
      selectableRange:
        options.limit === 'systemTimeBefore'
          ? `00:00:00-${moment(now).format('HH:mm:ss')}`
          : options.limit === 'systemTimeAfter'
          ? `${moment(now).format('HH:mm:ss')}-23:59:59`
          : options.limit === 'stepRange'
          ? `${moment(new Date(now - lessThan)).format('HH:mm:ss')}-${moment(
              new Date(now + moreThan)
            ).format('HH:mm:ss')}`
          : false
    }
  };
...
export function findListWithItem(selectItem, item) {
  const result = selectItem.columns.reduce((acc, column, index) => {
    const itemIndex = column.list.findIndex(
...
export function createRelatedFinder(array, targetKey = 'fieldConfigId') {
  ...
  for (const item of array) {
    ...
    if (key !== undefined && key !== null) {
      ...
      indexMap.get(key).push(...item.cascadeConfigs);
    }
  }

  // 返回查找函数
  return function findRelated(sourceItem, sourceKey = 'key') {
    if (!sourceItem || sourceItem[sourceKey] === undefined) {
      return [];
    }
    const keyValue = sourceItem[sourceKey];
    return indexMap.get(keyValue) || [];
  };
}

export function findCascadeRelations(data, targetId) {
  const result = [];

  function dfs(currentId) {
    for (const item of data) {
      if (Array.isArray(item.cascadeConfigs)) {
        for (const cfg of item.cascadeConfigs) {
          if (cfg.parentFieldConfigId === currentId) {
            result.push(cfg);
            // 递归查找以此字段为父的下一级
            dfs(cfg.fieldConfigId);
          }
        }
      }
    }
