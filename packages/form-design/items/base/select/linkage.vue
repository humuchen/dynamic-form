...
<template>
  <div class="dy-properties-linkage">
    <el-row :gutter="8">
      <span v-for="(val, index) in value" :key="index">
        <div class="linkage-box">
          <el-col :span="21">
            <template>
              {{ index + 1 }}、
              <el-radio v-model="val.vtype" :label="1">{{
                t('dyform.item.local')
              }}</el-radio>
              <el-radio v-model="val.vtype" :label="2">{{
                t('dyform.item.remote')
              }}</el-radio>
            </template>
          </el-col>
          <el-col :span="21">
            <el-input
              size="mini"
              v-model="val.model"
              :placeholder="t('dyform.item.linkage_column')"
            >
              <template slot="prepend">{{
                t('dyform.item.linkage_column')
              }}</template>
            </el-input>
          </el-col>
          <el-col :span="21">
            <template v-if="val.vtype == 1">
              {{ t('dyform.properties.script') }}:
              <el-input
                size="mini"
                type="textarea"
                v-model="val.script"
