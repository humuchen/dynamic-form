    ...
    >
      <template v-if="uploadVisible">
        <el-button
          ...
          size="small"
          type="primary"
          >{{ t('dyform.item.upload.select') }}</el-button
        >
        <i v-else class="el-icon-plus"></i>
        <div v-if="tip != undefined" slot="tip" class="el-upload__tip">
          {{ tip }}
        </div>
      </template>
    </el-upload>
  ...
  computed: {
    ...
    uploadResponseFileUrl() {
      if (
        this.record &&
        this.record.options &&
        this.record.options.responseFileUrl
      ) {
    ...
    uploadResponseFileId() {
      if (
        this.record &&
        this.record.options &&
        this.record.options.responseFileId
      ) {
  ...
  methods: {
    beforeUpload(file) {
      ...
      if (
        ...
        this.$message.error(
          this.t('dyform.item.upload.error_img_filetype') + '[png,jpg,jpeg,bmp]'
        );
        return false;
      }
      ...
      if (this.limitSize && ltSize > this.limitSize) {
        this.$message.error(
          this.t('dyform.item.upload.error_max_size') + this.limitSize + 'MB!'
        );
    ...
    fileDown(file) {
      if (file.url) {
        ...
        this.$message.error(this.t('dyform.item.upload.error_not_found_file'));
      }
    }
  }
};
...
<style>
.dy-form-upload.upload .el-upload {
  display: none;
}
</style>
