<template>
  <div class="dy-form-editor">
    ...
    <div v-if="previewVisible" class="wangEditor_html-content">
      ...
      <div style="overflow-y: auto; height: 100%" v-html="text"></div>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  watch: {
    ...
    placeholder(val) {
      if (this.editor) this.editor.config.placeholder = val
    },
    html(val) {
      if (!this.editor) return
      const findex = this.editor.config.menus.findIndex(
        (t) => t == 'editor-html'
      )
      if (val) {
        // 判断是否包含 不包含则添加
        if (findex < 0) {
          this.editor.config.menus =
            this.editor.config.menus.concat('editor-html')
        }
      } else {
        // 判断是否包含 包含则删除
        if (findex >= 0) {
  ...
  methods: {
    ...
    initEdit() {
      ...
      if (this.preview) {
        ...
        this.editor.config.menus =
          this.editor.config.menus.concat('editor-preview')
      }
