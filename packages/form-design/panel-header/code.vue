...
<template>
  <el-dialog
    :title="t('dyform.header.template_data')"
  ...
  >
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button v-if="edit" size="mini" type="primary" @click="handleImp">{{
        t('dyform.import')
      }}</el-button>
    </span>
  </el-dialog>
...
export default {
  ...
  methods: {
    ...
    handleImp() {
      if (!this.jsonCode) {
        this.$message.error(this.t('dyform.header.content_not_empty'));
        return;
      }
      try {
        ...
        if (importData && typeof importData == 'object') {
          ...
          this.$message.error(this.t('dyform.header.content_parse_error'));
          return;
        }
      } catch (error) {
        this.$message.error(this.t('dyform.header.content_parse_error'));
    ...
    exportData(data, fileName = `dy-form-template.${this.fileFormat}`) {
