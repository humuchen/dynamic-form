<template>
  <el-collapse class="drag-panel" v-model="actives">
    <template v-for="(colItem, colIndex) in dataList">
      <el-collapse-item
        :key="colIndex + formKey"
        :title="getLabel(colItem.name)"
        :name="colIndex + 1"
        v-if="colItem && colItem.list && colItem.list.length > 0"
      >
        <template slot="title">
    ...
    dataList() {
      ...
      items.map((t) => {
        if (t.type == 'basic' && this_.basicItem instanceof Array) {
          const listChildren = t.list.filter((n) =>
            this_.basicItem.includes(n.type)
          );

          t.list = listChildren;
        } else if (t.type == 'layout' && this_.layoutItem instanceof Array) {
          const listChildren = t.list.filter((n) =>
            this_.layoutItem.includes(n.type)
          );

          t.list = listChildren;
        } else if (
          t.type == 'application' &&
          this_.applicationItem instanceof Array
        ) {
          const listChildren = t.list.filter((n) =>
            this_.applicationItem.includes(n.type)
          );

          t.list = listChildren;
        } else if (
          t.type == 'decorate' &&
          this_.decorateItem instanceof Array
        ) {
          const listChildren = t.list.filter((n) =>
            this_.decorateItem.includes(n.type)
          );
      ...
      if (this.customComponents && this.customComponents.length > 0) {
        items.push({
          type: 'custom',
          name: this.$t('dyform.item.custom_components'), //'自定义组件',
          list: this.customComponents
        });
        // const decorateItems = items.find((t) => t.type == 'decorate');
        // if (decorateItems) {
        //   decorateItems.list = decorateItems.list.concat(this.customComponents);
        // }
      }
...
.drag-panel .el-collapse-item__header {
  height: 35px;
  line-height: 35px;
}
...
.drag-panel .el-collapse-item__header,
.drag-panel .el-collapse-item__wrap {
  border-bottom: none;
}
.drag-panel .el-collapse-item__content {
  padding-bottom: 0;
}
</style>
